...
//  const float v_sig = ci + cj - const_viscosity_beta * mu_ij;
  ...
  //magacc[i]       = mj * 
  pi->a_hydro[i] -= mj * 
  		   ((mm_i[i][0]*mag_faci+mm_j[i][0]*mag_facj)*dx[0]+
  	            (mm_i[i][1]*mag_faci+mm_j[i][1]*mag_facj)*dx[1]+
  	            (mm_i[i][2]*mag_faci+mm_j[i][2]*mag_facj)*dx[2]);
  const float divBTerm=0.0 * 
  		   (pi->Bfld[0]*mag_faci+pj->Bfld[0]*mag_facj)*dx[0]+
       	           (pi->Bfld[1]*mag_faci+pj->Bfld[1]*mag_facj)*dx[1]+
  	           (pi->Bfld[2]*mag_faci+pj->Bfld[2]*mag_facj)*dx[2];
  //(pi->Bfld[0]-pj->Bfld[0])*(mag_faci+mag_facj)/2*dx[0]+
  //	 	  (pi->Bfld[1]-pj->Bfld[1])*(mag_faci+mag_facj)/2*dx[1]+
  //		  (pi->Bfld[2]-pj->Bfld[2])*(mag_faci+mag_facj)/2*dx[2];
  for(int i=0;i<3;++i)
 // magcorr[i]      = mj * pi->Bfld[i] * (
  pi->a_hydro[i] += mj * pi->Bfld[i] * divBTerm;
  //(
  //		   (pi->Bfld[0]*mag_faci+pj->Bfld[0]*mag_facj)*dx[0]+
  //	           (pi->Bfld[1]*mag_faci+pj->Bfld[1]*mag_facj)*dx[1]+
  //	           (pi->Bfld[2]*mag_faci+pj->Bfld[2]*mag_facj)*dx[2]);
 // const float macc =magacc[0]*magacc[0]+magacc[1]*magacc[1]+magacc[2]*magacc[2];
 // const float mcorr=magcorr[0]*magcorr[0]+magcorr[1]*magcorr[1]+magcorr[2]*magcorr[2];
 // if(macc<mcorr)
 //    for(int i=0;i<3;++i)
 //       magcorr[i] *= macc/mcorr; 
 // 
 // for(int i=0;i<3;++i)
 //    pi->a_hydro[i] -= (magacc[i]-magcorr[i]);
  ...
  for(int i=0;i<3;++i)
  pj->a_hydro[i] -= mi * pj->Bfld[i] * divBTerm;
  //(
  //		   (pi->Bfld[0]*mag_faci+pj->Bfld[0]*mag_facj)*dx[0]+
  //	           (pi->Bfld[1]*mag_faci+pj->Bfld[1]*mag_facj)*dx[1]+
  //	           (pi->Bfld[2]*mag_faci+pj->Bfld[2]*mag_facj)*dx[2]);
